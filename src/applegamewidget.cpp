...
void AppleGameWidget::startGame()
{
	...
	if (!m_testMode && m_soundEnabled && m_bgmPlayer->state() != QMediaPlayer::PlayingState) {// 只有在非测试模式下才播放背景音乐
        m_bgmPlayer->play();
    }
}
...
void AppleGameWidget::onUpdateTimer()
{
	...
	if (m_testMode) {// 测试模式下根据预设字母生成，不随机生成
        shouldSpawn = false;
    } else {
        shouldSpawn = (m_random.bounded(SPAWN_PROBABILITY) < APPLE_SPAWN_RATE
            || m_apples.size() < m_random.bounded(1 + m_random.bounded(m_speedLevel) / 2))
            && m_apples.size() < m_maxAppleCount;
    }
...
{
    QChar letter;
	if (m_testMode && !m_testLetters.isEmpty()) 
    {// 测试模式下按顺序使用测试字母
        int idx = m_testLetterIndex;
        while (idx < m_testLetters.size()) {
            QChar c = m_testLetters.at(idx).toUpper();
            idx++;
            if (!c.isLetter()) continue;
            bool used = false;
            for (const Apple& a : m_apples) {
                if (a.letter == c) { used = true; break; }
            }
            if (!used) { letter = c; m_testLetterIndex = idx; break; }
        }
        ...
        letter = available.at(m_random.bounded(available.size()));
    }
    ...
    if (!text.isEmpty()) {
        ...
        if (ch.isLetter()) {
			...
			if (m_testMode) {//测试模式下新增逻辑：如果输入无效（没有移除苹果），增加错误计数
                ...
                while (m_apples.size() >= m_maxAppleCount) {
                    Apple& first = m_apples[0];
                    if (first.badTimer) { first.badTimer->stop(); first.badTimer->deleteLater(); }
                    m_apples.removeAt(0);
                }
                if (m_apples.size() < m_maxAppleCount) {
                    generateApple();
                }
            }
        }
...
//命令行测试模式相关接口，允许外部设置测试字母序列和开启测试模式（不随机生成苹果，按序生成指定字母）
void AppleGameWidget::setTestMode(bool enabled)
{
    m_testMode = enabled;
}

void AppleGameWidget::setTestLetters(const QString& letters)
{
    m_testLetters = letters;
    m_testLetterIndex = 0;
    m_wrongInputCount = 0;
}

void AppleGameWidget::setGameParameters(const QJsonObject& config)
{
    QJsonObject s = config["settings"].toObject();
    if (s.contains("speed")) m_speedLevel = s["speed"].toInt();
    if (s.contains("targetAppleCount")) m_targetAppleCount = s["targetAppleCount"].toInt();
    if (s.contains("maxBadAppleCount")) m_maxBadAppleCount = s["maxBadAppleCount"].toInt();
    if (s.contains("maxAppleCount")) m_maxAppleCount = s["maxAppleCount"].toInt();
}

QJsonObject AppleGameWidget::getGameStats() const
{
    QJsonObject stats;
    stats["health"] = QString("%1/%2").arg(getCurrentHealth()).arg(getMaxHealth());
    stats["missed"] = m_badCount;
    stats["correctInputs"] = m_successCount;
    stats["wrongInputs"] = m_wrongInputCount;
    stats["score"] = 0;
