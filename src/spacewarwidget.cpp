...
void SpaceWarWidget::spawnObject() {
    ...
    if (m_testMode) {
		spawnEnemy = (enemyCount < maxEnemy);//测试模式下只要未满额就生成敌机
    } else {
        spawnEnemy = (enemyCount < maxEnemy && QRandomGenerator::global()->bounded(100) < 3);
        spawnMeteor = (meteorCount < maxMeteor && QRandomGenerator::global()->bounded(100) < 2);
    }
...
QChar SpaceWarWidget::getUnusedLetter() {
	if (m_testMode && !m_testLetters.isEmpty()) {//测试模式下按顺序使用测试字母
        int idx = m_testLetterIndex;
        while (idx < m_testLetters.size()) {
            QChar c = m_testLetters.at(idx).toUpper();
            idx++;
            if (!c.isLetter()) continue;
            if (!m_usedLetters.contains(c)) {
                m_testLetterIndex = idx;
                return c;
            }
        }
        return QChar();
...
void SpaceWarWidget::updateObjects() {
    for (auto& obj : m_objects) {
        ...
        if (obj.pos.y() > height() + 50) {
            ...
            if (m_testMode) { m_missedCount++; m_lives--; }
...
void SpaceWarWidget::handleLetterInput(QChar letter) {
    ...
    for (auto& obj : m_objects) {
        if (!obj.exploded && obj.letter == letter) {
            ...
            if (m_testMode) m_correctInputCount++;
            return;
        }
    }
...
// 测试相关接口
void SpaceWarWidget::setTestMode(bool enabled)
{
    m_testMode = enabled;
}

void SpaceWarWidget::setTestLetters(const QString& letters)
{
...
void SpaceWarWidget::setGameParameters(const QJsonObject& config)
{
    QJsonObject s = config["settings"].toObject();
    if (s.contains("enemysNum")) { m_enemyMaxCount = s["enemysNum"].toInt(); m_meteoriteMaxCount = m_enemyMaxCount / 2; }
    if (s.contains("enemysSpeed")) m_speedLevel = s["enemysSpeed"].toInt();
    if (s.contains("upgradeInterval")) m_upgradeIntervalMs = s["upgradeInterval"].toInt() * 1000;
    if (s.contains("isPrize")) m_rewardEnabled = s["isPrize"].toBool();
}

QJsonObject SpaceWarWidget::getGameStats() const
{
    QJsonObject stats;
    stats["health"] = QString("%1/%2").arg(m_lives).arg(m_maxLives);
    stats["missed"] = m_missedCount;
    stats["correctInputs"] = m_correctInputCount;
    stats["wrongInputs"] = m_wrongInputCount;
    stats["score"] = m_score;
    int pw = 0, pc = 0;
    for (const auto& rw : m_rewardWords) {
        if (rw.finished) { pw++; pc += rw.text.size(); }
    }
    QJsonObject prize;
    prize["chars"] = pc;
    prize["words"] = pw;
    stats["prizeText"] = prize;
    return stats;
}