...
- **音效**：短音效用 `QSoundEffect`（低延迟预加载），背景音乐用 `QMediaPlayer` + `EndOfMedia` 信号循环
...
- **API Key 硬编码**：`spacewarwidget.cpp` 中 DeepSeek API Key 明文写在源码里，存在安全风险
...
- **命名不一致**：部分成员使用 `m_` 前缀，头文件中 `mouseReleaseEvent` 声明多了 `MainWindow::` 限定符
- **资源文件冗余**：`res.qrc` 是 `res_main.qrc + res_game.qrc` 的拼接，存在重复定义
- **SettingsDialog 生命周期**：析构时 delete，但重新创建时未先清理旧指针（非严重但不够严谨）
